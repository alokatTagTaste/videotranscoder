...
let promiseFor240 = {
    video: (timestamp, input_file_url, bucket_name) => {
        ...
        let promiseFor240Object = new Promise((resolve, reject) => {
            ...
            childProcess.on('exit', (code, signal) => {
                ...
                console.timeEnd('240p');
                if (code == 0) {
                    fs.readFile(__dirname + "/video_" + timestamp + "_240.mp4",
                        (err, data) => {
                            console.log("File", err, data);
                            if (err) {
                                reject(new Error("Error occured during transcoding 240p which causes to process exit with code `${code}` due to " + err.message));
                            } else {
                                console.log("is here..");
                                s3.putObject({
                                    Bucket: bucket_name,
                                    Key: "videotranscoder/240/video_" + timestamp + "_240.mp4",
                                    Body: data,
                                    ...
                                    if (err) {
                                        ...
                                        fs.unlink(__dirname + "/video_" + timestamp + "_240.mp4", (err) => {
                                            if (err) {
                                                reject(new Error("Error occured during transcoding 240p which causes to process exit with code `${code}` due to " + err.message));
                                            } else {
                                                resolve("Done");
        ...
        return promiseFor240Object;

    }
}

module.exports = { promiseFor240 };