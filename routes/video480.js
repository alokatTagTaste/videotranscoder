...
let promiseFor480 = {
    video: (timestamp, input_file_url, bucket_name) => {
        ...
        let promiseFor480Object = new Promise((resolve, reject) => {
            ...
            childProcess.on('exit', (code, signal) => {
                ...
                console.timeEnd('480p');
                if (code == 0) {
                    fs.readFile(__dirname + "/video_" + timestamp + "_480.mp4",
                        (err, data) => {
                            console.log("File", err, data);
                            if (err) {
                                reject(new Error("Error occured during transcoding 480p which causes to process exit with code `${code}` due to " + err.message));
                            } else {
                                console.log("is here..");
                                s3.putObject({
                                    Bucket: bucket_name,
                                    Key: "videotranscoder/480/video_" + timestamp + "_480.mp4",
                                    Body: data,
                                    ...
                                    if (err) {
                                        ...
                                        fs.unlink(__dirname + "/video_" + timestamp + "_480.mp4", (err) => {
                                            if (err) {
                                                reject(new Error("Error occured during transcoding 480p which causes to process exit with code `${code}` due to " + err.message));
                                            } else {
                                                resolve("Done");
            ...
            childProcess.on('error', (code, signal) => {
                ...
                reject(new Error("Error occured during transcoding 480p which causes to process exit with code `${code}`"));
            });
        ...
        return promiseFor480Object;

    }
}

module.exports = { promiseFor480 };