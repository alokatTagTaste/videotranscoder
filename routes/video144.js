...
let promiseFor144 = {
    video: (timestamp, input_file_url, bucket_name) => {
        ...
        let promiseFor144Object = new Promise((resolve, reject) => {
            ...
            childProcess.on('exit', (code, signal) => {
                ...
                console.timeEnd("144p");
                if (code == 0) {
                    // resolve("Done");
                    fs.readFile(__dirname + "/video_" + timestamp + "_144.mp4",
                        (err, data) => {
                            console.log("File", err, data);
                            if (err) {
                                reject(new Error("Error occured during transcoding 144p which causes to process exit with code `${code}` due to " + err.message));
                            } else {
                                console.log("is here..");
                                s3.putObject({
                                    Bucket: bucket_name,
                                    Key: "videotranscoder/144/video_" + timestamp + "_144.mp4",
                                    Body: data,
                                    ...
                                    if (err) {
                                        ...
                                        fs.unlink(__dirname + "/video_" + timestamp + "_144.mp4", (err) => {
                                            if (err) {
                                                reject(new Error("Error occured during transcoding 144p which causes to process exit with code `${code}` due to " + err.message));
                                            } else {
                                                resolve("Done");
            ...
            childProcess.on('error', (code, signal) => {
                ...
                reject(new Error("Error occured during transcoding 144p which causes to process exit with code `${code}`"));
            });
        ...
        return promiseFor144Object;

    }
}

module.exports = { promiseFor144 };